  ...
  return (
    <div className="map">
      ...
      <svg className="map-svg" viewBox="0 0 280 280" role="img" aria-label={`Concepts related to ${center.title}`}>
        <circle cx="140" cy="140" r="118" fill="none" stroke="currentColor" opacity="0.15" />
        {nodes.map((node, i) => {
          ...
          return (
            <g key={node.id}>
              <line x1="140" y1="140" x2={x} y2={y} stroke="currentColor" opacity="0.25" />
              <circle cx={x} cy={y} r="6" fill={node.domain === "tb" ? "#9e3140" : "#1e4854"} />
            </g>
          );
        })}
        <circle cx="140" cy="140" r="10" fill="#1a1612" />
      </svg>
